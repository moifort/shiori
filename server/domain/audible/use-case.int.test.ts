...
describe('correcting the genres an older mapping got wrong', () => {
  const LATER = new Date('2026-09-26T04:00:00.000Z')
  // Where Amazon files Le Nom du vent besides the catch-all literary rack.
  const fantasyLadders = [
    {
      root: 'Genres',
      categories: [
        { id: '21228884031', name: 'Littérature, romans et fiction' },
        { id: '21229019031', name: 'Fiction' },
      ],
    },
    {
      root: 'Genres',
      categories: [
        ...
        { id: '21229701031', name: 'Épique' },
      ],
    },
  ]
  const imported = (genre?: 'literary-fiction' | 'romance') =>
  ...
  test('says so when no account is connected', async () => {
    expect(await AudibleUseCase.correctImportedGenres(reader, LATER)).toBe('not-connected')
  })

  test('moves an import off the catch-all literary rack', async () => {
    await connect()
    await imported('literary-fiction')
    items = [anItem({ categories: fantasyLadders })]

    expect(await AudibleUseCase.correctImportedGenres(reader, LATER)).toBe(1)
    const [book] = await BookQuery.all(reader)
    expect(book?.genre).toBe('fantasy')
  })

  test('gives a genre to an import that landed with none', async () => {
    await connect()
    await imported()
    items = [anItem({ categories: fantasyLadders })]

    expect(await AudibleUseCase.correctImportedGenres(reader, LATER)).toBe(1)
    expect((await BookQuery.all(reader))[0]?.genre).toBe('fantasy')
  })

  // A genre the old mapping could not have produced by mistake is either right
  // or the reader's own choice.
  test('leaves any other genre alone', async () => {
    await connect()
    await imported('romance')
    items = [anItem({ categories: fantasyLadders })]

    expect(await AudibleUseCase.correctImportedGenres(reader, LATER)).toBe(0)
    expect((await BookQuery.all(reader))[0]?.genre).toBe('romance')
  })

  test('adopts an import made before the ASIN was kept, without linking it', async () => {
    await connect()
    await BookCommand.add(reader, {
      title: BookTitle('Le Nom du vent'),
      authors: [AuthorName('Patrick Rothfuss')],
    ...
    const [book] = await BookQuery.all(reader)
    expect(book?.genre).toBe('fantasy')
    expect(book?.audibleAsin).toBeUndefined()
  })

  // The migration runs this for every reader once. One revoked device must not
  // block every migration after it.
  test('runs once over every connected reader, stepping over a refusal', async () => {
    const { correctAudibleGenres } = await import(
      '~/system/migration/migrations/001-correct-audible-genres'
    )
    const other = 'reader-2' as UserId
    await connect()
    await connect(other)
    await AudibleCommand.setAutoSync(other, false)
    await imported('literary-fiction')
    libraryRefusals.push(undefined, new Error('device revoked'))
    items = [anItem({ categories: fantasyLadders })]

    expect(await correctAudibleGenres.migrate({ db: fakeDb() as never })).toEqual({
      ok: true,
      transformed: 1,
    })
